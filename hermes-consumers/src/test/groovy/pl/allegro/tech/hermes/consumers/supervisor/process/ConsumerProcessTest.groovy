...
class ConsumerProcessTest extends Specification {
    ...
    def "should process be unhealthy when last seen period is greater than unhealthy period"() {
        ...
        when:
        process.accept(Signal.of(Signal.SignalType.STOP, subscription.getQualifiedName()))
        waiter.waitForSignalProcessing()
    ...
    def "should tear down and initialize consumer on restart but not call shutdown hook"() {
        ...
        when:
        process.accept(Signal.of(Signal.SignalType.RESTART, subscription.getQualifiedName()))
        waiter.waitForSignalProcessing()
    ...
    def "should stop, retransmit and start consumer on retransmission"() {
        ...
        when:
        process.accept(Signal.of(Signal.SignalType.RETRANSMIT, subscription.getQualifiedName()))
        waiter.waitForSignalProcessing()
    ...
    def "should update subscription"() {
        ...
        when:
        process.accept(Signal.of(Signal.SignalType.UPDATE_SUBSCRIPTION, subscription.getQualifiedName(), modifiedSubscription))
        waiter.waitForSignalProcessing()
